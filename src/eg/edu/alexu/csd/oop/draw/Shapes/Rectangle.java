...
public class Rectangle implements Shape {
    ...
    Color fillColor;
    int strokeWidth;
    public Rectangle(Point a, Color color, Color fillColor, int strokeWidth){
        topLeft.x = a.x;
        topLeft.y = a.y;
        this.color = color;
    ...
    public boolean checkInside(Point p){
        ...
        return GeometryHandler.checkPointInRectangle(px, py, width, height, p);
    }
    public int getUUID(){
        return UUID;
    }
    ...
    public void draw(Graphics canvas) {
        ...
        if(StaticVars.selectedShape == this){
            ...
            canvas.drawRect(px-strokeWidth/2, py-strokeWidth/2, Math.abs(width)+strokeWidth, Math.abs(height)+strokeWidth);
        }
        canvas.setColor(color);
        ((Graphics2D) canvas).setStroke(new BasicStroke(strokeWidth));
        canvas.drawRect(px, py, Math.abs(width),Math.abs(height));
        canvas.setColor(fillColor);
        canvas.fillRect(px, py, Math.abs(width),Math.abs(height));
    }

    @Override
    public Object clone() throws CloneNotSupportedException {
        Rectangle temp = new Rectangle(topLeft, new Color(color.getRGB()), new Color(fillColor.getRGB()), strokeWidth);
